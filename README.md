...
[Zerodha Support](https://support.zerodha.com) — publicly available FAQ knowledge base for India's largest retail discount brokerage firm. All data is scraped from public URLs with polite rate-limiting. 
...
The author of this repo neither owns this data nor guarantees the correctness of it. 
The outputs are generated by a large language model. They are not verified by any domain expert. Data accuracy, completeness, and timeliness are not guaranteed. The model may hallucinate values, misread sources, or present outdated information as current. 
The author of this tool is not a registered investment advisor and bears no responsibility for any financial losses, trading decisions, or investment outcomes arising from the use of, or reliance on, any output produced by this tool. By using this tool, you acknowledge that you are doing so entirely at your own risk. For guidance on how to receive regulated financial advice in India, visit [SEBI](https://www.sebi.gov.in/) and identify a registered advisor appropriate for your needs.
